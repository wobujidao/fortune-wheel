...
<p align="center">
  <img src="assets/bot-avatar.png" width="160" alt="Колесо Фортуны" />
</p>
...
> Вероятности: все секторы равновероятны (`secrets.choice` — криптографический генератор).
...
- **Режим разработки** — включается в админке, позволяет крутить колесо бесконечно без сохранения результатов (для тестирования анимации).
...
Отдельная веб-страница (`/admin`), доступ по Telegram-авторизации. Mobile-first: на экранах <600px таблицы автоматически превращаются в карточки, табы со скроллом, toast-уведомления, кастомные диалоги подтверждения.
...
| **Просмотрщик** | Просмотр результатов и призов (без кнопок действий) |

### Управление призами

- ➕ Добавить приз (текст, иконка-эмодзи, цвет, позиция)
- ✏️ Редактировать приз (текст, иконка, цвет, позиция, активность)
- 🗑️ Удалить приз
- ↕️ Drag & drop сортировка (перетаскивание строк)
- Поддержка от 2 до 12 секторов

### Управление результатами

- 📊 Таблица результатов (кто, что, когда, Telegram-данные)
- 📈 Статистика: графики распределения призов и вращений по дням
- 🔍 Поиск и фильтрация
...
- 📋 Список текущих админов и просмотрщиков с именами (подтягиваются через Telegram Bot API)

### Аудит-лог

- 📋 Вкладка «Лог» — история всех действий администраторов
- Записывается: дата, админ, действие, детали
- Отслеживаемые действия: удаление/сброс вращений, CRUD призов, сортировка, добавление/удаление пользователей

### Настройки

- ⚙️ Вкладка «Настройки» в админке
- Стиль указателя колеса: «Сверху» (стрелка сверху, остриём к центру) или «Из центра» (стрелка из хаба вверх к призу)
- Настройки хранятся в БД (таблица `settings`, key-value), применяются глобально для всех пользователей

### Режим разработки

- 🛠️ Toggle-переключатель во вкладке «Настройки» админки
- Колесо крутится бесконечно без обращения к API
- Результаты **не сохраняются** в БД
- Визуальный баннер на колесе при включённом режиме
- Флаг передаётся через `localStorage` (один origin)
...
- Золотая стрелка-указатель: два стиля на выбор (сверху — остриём к центру; из центра — вверх к призу). Переключается в админке
- Легенда с расшифровкой номеров под колесом
- Конфетти при выигрыше
- **Mobile-first** — компактная вёрстка: колесо, легенда и кнопка помещаются на одном экране без скролла. Размер колеса адаптируется под высоту viewport (`dvh`), внутренние элементы масштабируются пропорционально через процентное позиционирование
- Колесо строится динамически — количество секторов, цвета и тексты загружаются из API

### UX-улучшения

- **Loading-спиннер** — золотой анимированный спиннер при загрузке, fade-out по готовности
- **Toast-уведомления** — всплывающие сообщения об ошибках (полупрозрачный блок, автоскрытие 4 сек)
- **HapticFeedback** — тактильная отдача при нажатии кнопки (medium), смене сектора (light), выигрыше (success)
- **Звуковые эффекты** — Web Audio API (без файлов): тиканье при секторах, восходящие фанфары при выигрыше. Кнопка 🔊/🔇 (сохраняется в localStorage)
- **BackButton** — Telegram BackButton закрывает модалку результата
...
```mermaid
graph LR
    A[🌍 Интернет] -->|порты 80 / 443| B[🔀 MikroTik<br/>95.165.142.229]
    B -->|NAT| C[🔒 Nginx Proxy Manager<br/>192.168.5.4]
    C -->|fortune.demidov.info<br/>reverse proxy| D[🐳 cbmo-fortune<br/>192.168.5.11:8000]
    E[📱 Telegram API] <-->|polling| D

    subgraph Proxmox
        C
        D[🐳 Ubuntu 24.04 LXC<br/>192.168.5.11]
    end
```
...
| Web-сервер | Nginx (reverse proxy + статика) |
| Деплой | Docker + Docker Compose |
| Авторизация | Telegram WebApp initData (HMAC-SHA256) |
| Рандом | `secrets.choice` (OS CSPRNG) |
| Линтер | Ruff |
...
│   │   └── models.py           # SQLAlchemy-модели (prizes, spins, admins, audit_log, settings)
│   ├── handlers/
│   │   ├── __init__.py
...
- **busy_timeout 5с** — при блокировке БД запрос ожидает до 5 секунд вместо мгновенной ошибки
...
| `GET` | `/api/admin/audit` | Аудит-лог действий администраторов |
| `PUT` | `/api/admin/settings` | Обновить настройку (key + value) |
...
| `/admin` | Админ | WebApp-кнопка для открытия админки |
